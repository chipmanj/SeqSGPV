...
Within the estimated effect is within the peripheral region, the unbiased expected wait time is the n to achieve a desired margin of error width (as specified among the sgpvAM inputs).  When the estimated effect is outside the peripheral region, a wider confidence interval width is permitted -- thus allowing studies with more extreme effects to begin monitoring and conclude stopping earlier.

If estimate is within peripheral, ME wait time = input into sgpvAM function.

If estimate is outside peripheral, ME wait time = input into sgpvAM function + distance to closest peripheral boundary.

Starting time to monitoring is itself an adaptive process and will bias to starting too early.  Hence, we raise an alert whenever the estimated Margin of Error is less than the wait time Margin of Error width, and wait until the alert is affirmed 30 patients later.
