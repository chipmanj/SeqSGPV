...
The following rule is still under developmental yet makes use of the theoretical properties explore when the true standard deviation is known.  It also addresses the two limitations.  

First, we define a periphrial region of effects around the boundary Regions of Clinically Trivial and Clinically Highly Actionable Effects.  Within this region, we do not relax the minimum wait time.  For a one-sided study where a positive outcome is beneficial, this region is [point null, deltaG1 + deltaG2]; a one-sided study where a negative effect is beneficial, the region is [deltaL2 + deltaL1, point null]; a two-sided study, the region is [deltaL2 + deltaL1, deltaG1 + deltaG2].  
