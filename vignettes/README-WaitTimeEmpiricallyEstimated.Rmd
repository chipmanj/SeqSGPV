...
**Wait time until monitoring:** Current recommendations for wait time are based upon the minimum sample size to achieve an expected minimum 95% margin of error width of the treatment effect under an assumed outcome's standard deviation.  This has optimal properties for reducing the Type I Error.  This assumes a working knowledge of the outcome's variability.  While optimal for reducing the Type I Error, there are two draw backs: 1) the true standard deviation may not be known and 2) extreme effects are followed for a minimum sample size which may be much longer than needed to stop the trial early.

The following rule is still under developmental yet makes use of the optimal properties explored when the true standard deviation is known.  It also relaxing the requirement of knowing the true standard deviation, and it allows studies with fringe effects to terminate more quickly.

First, we define a peripheral region of effects around the boundary Regions of Clinically Trivial and Clinically Highly Actionable Effects.  In a two-sided study, this can be visualized as the non-highly actionable effects plus a small set of highly actionable effects.  More formally, the two-sided region is [deltaL2 + deltaL1, deltaG1 + deltaG2].  In a one-sided study where a positive effect is beneficial, the peripheral region includes the moderately actionable effects plus a small set of trivial and a small set of highly actionable effects.  Specificially, this region is [point null, deltaG1 + deltaG2].  Similarly, when a negative effect is beneficial, the peripheral region is [deltaL2 + deltaL1, point null].


```{r periph, echo=FALSE, fig.pos = "H",fig.cap="Left - One-sided hypothesis with the Region of At Most Trivial Effects and Region of Highly Actionable Clinical Effects.  Right - Two-sided hypotheis with Regions of Trivial Effects and Highly Actionable Clinical Effects.  The red boxes encapsulate periphreal effects surrounding the boundaries of the regions the regions.  "}
knitr::include_graphics("~/Dropbox/statistics/work/vandy/AdaptiveMonitoring/manuscript/AMwithSGPV_Figsv04/Slide06.jpg")
```

Within the estimated effect is within the peripheral region, the expected wait time is the n to achieve a desired confidence interval width (as specified among the sgpvAM inputs).  When the estimated effect is outside the peripheral region, a wider confidence interval width is permitted -- thus allowing studies with more extreme effects to begin monitoring and conclude stopping earlier.

When the i$^th$ patient enrolls, the estimated treatment effect and confidence interval are calculated.  Conditioned on the estimated standard deviation of the outcome

When the estimated effect is within this peripheral region, the estimated wait time after observing the i$^{th}$ patient is the n for when the confidence interval width, conditioned on estimated variability, is the specified waitWidth input parameter.  The first possible time to start monitoring is when the estimated wait time is less than the number of observations.  Given the distrecte brownian motion in estimating both the effect and variability, we require an affirmation to start monitoring.  Currently, this is to wait until 10 patients later the data still suggest to begin monitoring.  The affirmation step to begin monitoring reduces the chances of some mcmc replicates beginning to monitor too early and ending having made an error.

When the estimated effect is outside this peripheral region, the estimated wait time after observing the i$^{th}$, the required waitWidth is relaxed.  The wait width allows to be wider by half the distance between the estimated effect and the boundary of the periphreal region.  This allows strongly positive or negative studies to end earlier with a wider confidence interval.

Because of greater variability in when MCMC replicates begin monitoring, additional replicates may be needed to get a stable estimate of operating characteristics.
...
Suggest to start small with 2000 reps to explore study designs.
...
system.time(am <-  sgpvAM(nreps            = 2000,
                          ...
                          minWaitN         = 30,
                          kSteps           = 10,
                          waitWidths       = seq(0.2, mean(c(.2,.50)), length.out=5),
...
plot(amShifted, "mse",               alertK = 20,      xlim=c(-0.5, 1),   ylim=c(0,0.6))
