  ...
  if(! is.null(existingData) ) {
    eci <- rbind(existingData[,c("est","lo","up")],
                t(sapply( (length(y)-dataGenArgs[["n"]] + 1):length(y),
                          mdlFit, y=y, trt=trt, miLevel = monitoringIntervalLevel)))
  } else {
    eci <- rbind(matrix(rep(c(NA,-10^10,10^10),4),byrow = TRUE,nrow=4),
                 t(sapply(5:length(y), mdlFit, y=y, trt=trt, miLevel = monitoringIntervalLevel )))
...
amData <- function(nreps, fork=TRUE, socket = TRUE, cores, ... ){
  ...
  if(fork==TRUE){
    ...
    mcmcMonitoring <- parallel::mclapply(1:nreps, amDataSingle,
                                         dataGeneration   = dataGeneration,   dataGenArgs   = dataGenArgs,
                                         effectGeneration = effectGeneration, effectGenArgs = effectGenArgs,
                                         mdlFit         = mdlFit,         mdlFitArgs  = mdlFitArgs,
                                         monitoringIntervalLevel = monitoringIntervalLevel,
                                         existingData     = NULL, mc.cores      = cores)
    #
    # a = amDataSingle(dataGeneration   = dataGeneration,   dataGenArgs   = dataGenArgs,
    #              effectGeneration = effectGeneration, effectGenArgs = effectGenArgs,
    #              mdlFit         = mdlFit,         mdlFitArgs  = mdlFitArgs,
    #              monitoringIntervalLevel = monitoringIntervalLevel,
    #              existingData     = NULL)
...
system.time(a <- amData(nreps=20, dataGeneration   = dataGeneration,   dataGenArgs   = dataGenArgs,
                        effectGeneration = effectGeneration, effectGenArgs = effectGenArgs,
                        mdlFit         = mdlFit,         mdlFitArgs  = mdlFitArgs,
                        monitoringIntervalLevel = monitoringIntervalLevel,
                        existingData     = NULL, cores=detectCores()))

system.time(a <- amDataSingle(dataGeneration   = dataGeneration,   dataGenArgs   = dataGenArgs,
                        effectGeneration = effectGeneration, effectGenArgs = effectGenArgs,
                        mdlFit         = mdlFit,         mdlFitArgs  = mdlFitArgs,
                        monitoringIntervalLevel = monitoringIntervalLevel,
                        existingData     = NULL))
