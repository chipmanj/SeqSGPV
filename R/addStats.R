...
addStats <- function(o, pointNull, deltaL2, deltaL1, deltaG1, deltaG2){
  ...
  if(!anyNA(c(deltaL2, deltaL1, deltaG1, deltaG2))){
    ...
    sgpvROPE  <-     sgpv::sgpvalue(est.lo = o[,"lo"], est.hi = o[,"up"], null.lo =  deltaL1, null.hi =   10^10)$p.delta
    sgpvROME  <-     sgpv::sgpvalue(est.lo = o[,"lo"], est.hi = o[,"up"], null.lo =   -10^10, null.hi = deltaL2)$p.delta
  } else if(!anyNA(c(deltaG1, deltaG2))){

    # One sided: efficacy when greater than null
    sgpvROPE  <-     sgpv::sgpvalue(est.lo = o[,"lo"], est.hi = o[,"up"], null.lo =  -10^10, null.hi = deltaG1)$p.delta
    sgpvROME  <-     sgpv::sgpvalue(est.lo = o[,"lo"], est.hi = o[,"up"], null.lo = deltaG2, null.hi =   10^10)$p.delta
  } else{
  ...
  cbind(o, bias, rejPN, cover, sgpvROPE, sgpvROME)
