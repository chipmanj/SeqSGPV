...
sgpvAM <- function(mcmcData=NULL,
  ...
  if(is.null(mcmcData)){
         ...
         mcmcMonitoring <- amData(nreps=nreps,
                                  ...
                                  modolFit=lmCI,           mdlFitArgs=mdlFitArgs,
                                  existingData=NULL,
                                  monitoringIntervalLevel=monitoringIntervalLevel,
                                  fork=TRUE, socket = TRUE)
  ...
  if(!is.null(mcmcData) & sum(getMore) > 0){
    ...
    while(sum(getMore) > 0){
      ...
      # Update the number of observations to generate
      dataGeneration$n <- max(getMore)
      ...
      for (i in getMoreWhich){
        ...
        #                                     ... )
        mcmcMonitoring[[i]] <- amDataSingle(dataGeneration,   dataGenArgs,
                                            effectGeneration, effectGenArgs,
...
nreps = 20;
dataGeneration = rnorm;   dataGenArgs = list(n=800);
effectGeneration = 0; effectGenArgs=NULL;
mdlFit=lmCI;           mdlFitArgs=NULL;
monitoringIntervalLevel=0.05;
pointNull = 0
deltaL2 = -0.4; deltaL1=-0.3; deltaG1=0.3; deltaG2=0.4;
maxAlertSteps=100; lookSteps=1;
...
fork=TRUE;socket=TRUE;cores = detectCores()
